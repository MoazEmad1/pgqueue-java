...
- [x] **M3b (out-of-spec variant, see [`docs/m3b.md`](docs/m3b.md))** — SUE-mode ATTACH sweeper. Confirms the ATTACH-fixes-CREATE hypothesis (0 ATTACH rows blocked across 2700 s), still collapses (`t_50 = 497 s`, earlier than M3's 549 s). Three signals contribute — antagonist snapshot, dead-tuple accumulation, blocking overlay — and this run cannot cleanly separate them; details in the M3b section.
...
- **`descent_duration_seconds = 198`** (M3 = 60, R1 = 513, R2 = 577, R3 = 695). A progressive ratchet — sweeper stalls piling up over time on top of dead-tuple accumulation under a held xmin — predicts a slower decay than a single-shot binary wedge that pins the workload at zero. M3b's 198 s sits between M3's cliff (60 s) and the R1-family slow drift (513–695 s), consistent with a slower ratchet than R1 rather than the instant wedge M3 produced. (Shape of the bloat itself does NOT match R1 — see the storage numbers below.)
