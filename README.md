...
**M3 verdict.** Does not prevent collapse. `B = 1324 jobs/sec`, `t_50 = 549 s`, `t_25 = 609 s`, `final_ratio = 0.0000`. Pre-antagonist the sweeper works as designed: partitions fill, seal, and drop; throughput holds near baseline. The moment the antagonist starts (t = 300 s) throughput enters a sustained fall to zero, and by t ≈ 667 s the workload-metrics probes stop producing new values — `results/M3.meta.json` records a stale window covering t = 667 s to t = 2699 s (2032 s).

**Original hypothesis** — before the lock collector existed. The M3 wedge was believed to be a three-way contest between the sweeper's `DROP TABLE` needing `AccessExclusive` on the partition tree, blocked autovacuum holding `ShareUpdateExclusive` on partitions it cannot finish, and worker `SKIP LOCKED` traffic holding `RowShare`. The write-up further claimed the DB became unresponsive to any query. Both parts were wrong.

**What the lock graph shows.** With the two-probe superuser LockCollector running on `results/M3.locks.slow.csv` (rerun of 2026-08-04), the chain at t = 1000 s is:

- **PID 1130** — the antagonist. `SELECT count(*) FROM pgqueue.jobs`, `state = idle in transaction`, `query_age = 700 s`. Holds 22 relation-level locks, all `AccessShareLock`: on `pgqueue.jobs` (**the parent**), on all five currently-attached partitions, on each of their `_pkey` and `_state_created_at_idx` indexes, on the parent's own `jobs_pkey` and `idx_jobs_claimable`, plus a `virtualxid ExclusiveLock` (its own transaction).
- **PID 77** — the Sweeper. Query: `CREATE TABLE IF NOT EXISTS pgqueue.jobs_p_1785832440 PARTITION OF pgqueue.jobs …`. Waits for `AccessExclusiveLock` on `pgqueue.jobs`, `granted = false`, `blocked_by = 1130`.
- **PIDs 75, 76, 78, 79, …** — the 20 workers. Query: `UPDATE pgqueue.jobs SET state = 'claimed', claimed_at = now() WHERE id = ( … )`. All `blocked_by = 77`, queued behind the Sweeper's pending `AccessExclusive` on the parent.

The mechanism is not the DROP path. It is the **CREATE-partition path**. `CREATE TABLE … PARTITION OF parent` requests `AccessExclusive` on the parent, which conflicts with the antagonist's `AccessShareLock` on the parent that it acquired during its initial `SELECT count(*)` and — under `REPEATABLE READ` — never released. Every worker `UPDATE` needs `RowExclusive` on the parent's relation entry, which queues behind the Sweeper's pending `AccessExclusive`. Twenty workers stack up behind one Sweeper stack up behind one antagonist. Throughput collapses.

The DROP path is also unable to succeed while the antagonist holds `AccessShare` on the parent (same conflict), but in this run it is not what wedges the workload — the Sweeper's future-partition maintenance runs first and hits the wall first.

**Correction on "DB stops responding".** The 2032 s stale window in `M3.meta.json` reflects the workload's own metrics collector queries queuing behind the same lock contention the workers hit; it does not mean the database itself froze. The `pgqueue` superuser LockCollector, running on its own dedicated connection outside the Hikari pool, wrote 5399 fast-probe samples and 1350 slow-probe samples across the whole 2700 s run without a single reconnect or timeout (`results/M3.env.json`: `disconnected_seconds_total = 0`, `fast_query_timeout_count = 0`, `slow_query_timeout_count = 0`). The collector's queries touched only `pg_locks`, `pg_stat_activity`, and system catalogs — no locks on `pgqueue.*` — and completed with `max query_ms = 248 ms` fast and `133 ms` slow. The DB was responsive throughout; the workload was not.

**Follow-up spec (not run).** `docs/m3b.md` documents an alternative Sweeper implementation that creates partitions as standalone tables and then attaches them with `ALTER TABLE … ATTACH PARTITION`, which since PG12 takes `ShareUpdateExclusive` on the parent instead of `AccessExclusive`. That lock mode does not conflict with the antagonist's `AccessShare`, so the CREATE-partition chain observed above should not form. The DROP path remains blocked, so old partitions would accumulate for the antagonist's lifetime rather than clearing; the mitigation value would be throughput preservation, not storage reclamation. The spec sits outside the pre-registered `experiment.md` and awaits explicit approval to run.
